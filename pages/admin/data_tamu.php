...
// Query untuk tamu masuk terakhir hari ini (optional, bisa kamu sesuaikan lagi)
$hariIni = date('Y-m-d');
$queryTamuTerakhir = "
    (SELECT id, nama_tamu AS nama, status, waktu_checkin AS waktu_masuk, 'Unit A-01' AS tujuan_unit
     FROM tamu
     WHERE DATE(waktu_checkin) = '$hariIni')
    UNION ALL
    (SELECT id, nama_pengantar AS nama, 'Kurir' AS status, waktu_masuk, tujuan_unit
     FROM tamu_kilat
     WHERE DATE(waktu_masuk) = '$hariIni')
    ORDER BY waktu_masuk DESC
    LIMIT 10
";
$resultTamuTerakhir = $conn->query($queryTamuTerakhir);
if (!$resultTamuTerakhir) {
    echo "Query error tamu terakhir: " . $conn->error;
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <style>
    ...
    .sidebar { background-color: #3d7b65; min-height: 100vh; padding: 20px; color: #e0f0e6; width: 220px; position: fixed; top: 0; left: 0; }
