...
$stmt = $conn->prepare("INSERT INTO tamu (nama_tamu, no_identitas, tujuan, jenis_kelamin, no_hp, alamat, foto, status, waktu_checkin) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
if (!$stmt) {
    die("Prepare failed: (" . $conn->errno . ") " . $conn->error);
}
...
if ($stmt->execute()) {
    // Ganti redirect ke validasi_manual.php
    header("Location: validasi_manual.php?success=1&nama=" . urlencode($nama));
    exit;
} else {
